...
setwd("/home/ghislain/Code/forecasting-deforestation-Mada")
library(knitr)
...
You can execute Python code within a R notebooks. We first need to indicate which version of Python to use in the notebook for Python chunk.

```{r python_R}
Sys.setenv(PATH=paste("/usr/bin/python3",
           Sys.getenv("PATH"), sep=":"))
```
...
fcc_iCAR_2017 <- far$deforest(input_raster="output/pred_binomial_iCAR.tif",
...
- null model
- nsre model: no spatial random effect model
...
mod_dev <- data.frame(model=c("null", "nsre", "icar", "full"),
                  deviance=round(dev))
...
Model (nsre) + environmental variables (raster) => probabilities
...
#### Predicted fcc with nsre

Probabilities (raster) + deforested areas 2010-2017 (871336 ha) => prediction of forest cover change (fcc) 2010-2017

```{r fcc_2010_2017_nsre, results="hide"}
# Model predictions 2010-2017
fcc_nsre_2017 <- far$deforest(input_raster="output/pred_nsre.tif",
                              hectares=count_d$area,
                              output_file="output/fcc_2010_2017_nsre.tif",
                              blk_rows=128L)
```
...
```{r indices_2010_2017_nsre}
indices_2010_2017_nsre <- far$validation(pred="output/fcc_2010_2017_nsre.tif",
                                         obs="output/fcc_2010_2017_obs.tif",
                                         blk_rows=128L)
as.data.frame(indices_2010_2017_nsre)
```

### Differences in projections in 2050

Objective: compare predictions in 2050 for iCAR and nsre

#### Predictions in 2050 of nsre

```{r fcc_2010_2050_nsre, results="hide"}
# Model predictions 2010-2017
fcc_nsre_2050 <- far$deforest(input_raster="output/pred_nsre.tif",
                              hectares=4000000,
                              output_file="output/fcc_2010_2050_nsre.tif",
                              blk_rows=128L)
```
...
far$differences(inputA = "output/forest_cover_2050.tif",
                inputB = "output/fcc_2010_2050_nsre.tif",
                output_file = "output/diff_iCAR_nsre_2050.tif",
                blk_rows = 1L)
...
knitr::include_graphics("output/diff_iCAR_nsre_2050.png")
```

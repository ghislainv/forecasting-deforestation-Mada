...
opts_knit$set(
  root.dir=normalizePath("/home/ghislain/Code/forecasting-deforestation-Mada")
)
```
...
```{r plot_predictions}
if (!file.exists("output/prob_binomial_iCAR.png")) {
  fig <- far$plot$prob("output/prob_binomial_iCAR.tif",
                       output_file="output/prob_binomial_iCAR.png",
                       figsize=c(4,4))
}
knitr::include_graphics("output/prob_binomial_iCAR.png")
```
...
if (!file.exists("output/forest_cover_2050.tif")) {
  deforest <- far$deforest(input_raster="output/prob_binomial_iCAR.tif",
                           hectares=4000000,
...
```{bash resample}
if [ ! -f output/forest_cover_2050_epsg3857_ov32.tif ]
then
...
knitr::include_graphics("training/figures/Pontius2008-ARS-FOM.png")
```
...
```{bash fcc_2010_2017_valid}
if [ ! -f output/fcc_2010_2017_obs.tif ]
then
...
gdal_calc.py --overwrite -A output/fordefor2010_.tif -B output/for2017_.tif --outfile=output/fcc_2010_2017_obs.tif --type=Byte --calc='255-254*(A==1)*(B==1)-255*(A==1)*(B==255)' --co 'COMPRESS=LZW' --co 'PREDICTOR=2' --NoDataValue=255
fi
```
...
count_d <- far$countpix("output/fcc_2010_2017_obs.tif", value=0L, blk_rows=128L)
```
...
Note: Because forest covers about 9.3 Mha in 2010 [@Vieilledent2018], this corresponds to a total deforestation rate of about 9% on the period 2010-2017, and to an annual deforestation rate of about 1.34%.yr$^{-1}$. 

```{r fcc_2010_2017_iCAR, echo=TRUE, results="hide"}
# Model predictions 2010-2017
if (!file.exists("output/fcc_2010_2017_iCAR.tif")) {
  fcc_iCAR_2017 <- far$deforest(input_raster="output/prob_binomial_iCAR.tif",
                                hectares=count_d$area,
                                output_file="output/fcc_2010_2017_iCAR.tif",
                                blk_rows=128L)
}
```
...
```{r indices_2010_2017}
indices_2010_2017_iCAR <- far$validation(pred="output/fcc_2010_2017_iCAR.tif",
...
mod_glm <- smf$glm(formula_glm, r_to_py(data_train), family=sm$families$Binomial(), eval_env=-1L)$fit()
# Summary glm
...
performance_glm <- performance_index(data_valid,mod_glm)
performance_rf <- performance_index(data_valid,mod_rf)
performance_rf_xy <- performance_index(data_valid,mod_rf_xy)
# Save
write.table(performance_null, file="output/performance_null.txt", sep="\t", row.names=FALSE)
write.table(performance_glm, file="output/performance_glm.txt", sep="\t", row.names=FALSE)
write.table(performance_rf, file="output/performance_rf.txt", sep="\t", row.names=FALSE)
write.table(performance_rf_xy, file="output/performance_rf_xy.txt", sep="\t", row.names=FALSE)
write.table(performance_iCAR, file="output/performance_iCAR.txt", sep="\t", row.names=FALSE)
# Save for perc=10%
...
if (!file.exists("output/prob_rf.tif")) {
  # Compute predictions with logistic regression
  fig_prob_rfxy <- far$predict_raster(mod_rf, var_dir="data/model",
                                      input_forest_raster="data/model/fordefor2010.tif",

...
```{r prob_rfxy, results="hide"}
if (!file.exists("output/prob_rfxy.tif")) {
  # Compute predictions with logistic regression
  fig_prob_rf <- far$predict_raster(mod_rfxy, var_dir="data/model",
                                    input_forest_raster="data/model/fordefor2010.tif",
                                    output_file="output/prob_rfxy.tif",
                                    blk_rows=128L)
}
```

#### Project forest cover change on 2010-2017

```{r proj2017_glm, results="hide"}
...
```{r proj2017_rfxy, results="hide"}
if (!file.exists("output/proj2017_rfxy.tif")) {
  deforest <- far$deforest(input_raster="output/prob_rfxy.tif",
                           hectares=871336,
                           output_file="output/proj2017_rfxy.tif",
                           blk_rows=128L)
}
```
...
```{r tval_rfxy}
if (!file.exists("output/val_npix_rfxy.txt")) {
	tval_rfxy_df <- far$validation_npix(r_pred="output/proj2017_rfxy.tif",
																		 r_obs="output/proj2017_obs.tif",
																		 output_file="output/val_npix_rfxy.txt", 
																		 value_f=1,
																		 value_d=0,
																		 square_size=33L)
}
tval_rfxy_df <- read.table("output/val_npix_rfxy.txt",header=TRUE,sep=",")
e <- extent(raster("output/proj2017_rfxy.tif"))
cor_rfxy <- cor_scale(df=tval_rfxy_df, e=e, square_size=33)
```
...
cor_comp <- cor_glm %>%
	...
	dplyr::mutate(cor_rf=cor_rf$cor,cor_rfxy=cor_rfxy$cor,cor_icar=cor_icar$cor) %>%
	dplyr::select(-coeff)
readr::write_csv(cor_comp,path="output/cor_comp.txt")
cor_comp
```

#### Project forest cover change on 2010-2050

```{r proj2050_glm, results="hide"}
...
```{r proj2050_rf, results="hide"}
if (!file.exists("output/proj2050_rf.tif")) {
  deforest <- far$deforest(input_raster="output/prob_rf.tif",
                           hectares=4000000,
                           output_file="output/proj2050_rf.tif",
                           blk_rows=128L)
}
```

```{r proj2050_rfxy, results="hide"}
if (!file.exists("output/proj2050_rfxy.tif")) {
  deforest <- far$deforest(input_raster="output/prob_rfxy.tif",
                           hectares=4000000,
                           output_file="output/proj2050_rfxy.tif",
                           blk_rows=128L)
}
```

#### Comparing projection maps in 2050

First we compute the differences between model projections for 2050.
...
mat_icar_glm <- far$mat_diffproj("output/diff2050_icar_glm.tif")
index_icar_glm <- accuracy_indices(mat_icar_glm)
```

```{r diff2050_icar_rf}
if (!file.exists("output/diff2050_icar_rf.tif")) {
  far$r_diffproj(inputA="output/proj2050_icar.tif",
                 inputB="output/proj2050_rf.tif",
                 output_file="output/diff2050_icar_rf.tif",
                 blk_rows=1L)
}
mat_icar_rf <- far$mat_diffproj("output/diff2050_icar_rf.tif")
index_icar_rf <- accuracy_indices(mat_icar_rf)
```

```{r diff2050_icar_rfxy}
if (!file.exists("output/diff2050_icar_rfxy.tif")) {
  far$r_diffproj(inputA="output/proj2050_icar.tif",
                 inputB="output/proj2050_rfxy.tif",
                 output_file="output/diff2050_icar_rfxy.tif",
                 blk_rows=1L)
}
mat_icar_rfxy <- far$mat_diffproj("output/diff2050_icar_rfxy.tif")
index_icar_rfxy <- accuracy_indices(mat_icar_rfxy)
```

```{r diff2050_rfxy_rf}
if (!file.exists("output/diff2050_rfxy_rf.tif")) {
  far$r_diffproj(inputA="output/proj2050_rfxy.tif",
                 inputB="output/proj2050_rf.tif",
                 output_file="output/diff2050_rfxy_rf.tif",
                 blk_rows=1L)
}
mat_rfxy_rf <- far$mat_diffproj("output/diff2050_rfxy_rf.tif")
index_rfxy_rf <- accuracy_indices(mat_rfxy_rf)
```

We summarize the results in a single table

```{r diff_comp}
diff2050_comp <- dplyr::bind_rows(index_icar_glm, index_icar_rf,
                             index_icar_rfxy, index_rfxy_rf) %>%
  dplyr::mutate(comp=c("icar_glm","icar_rf","icar_rfxy","rfxy_rf")) %>%
	dplyr::select(8,1:7)
...
```{r diff2050_icar_glm_plot}
far$plot$differences("output/diff2050_icar_glm.tif",
                     borders="data/mada/mada38s.shp",
                     output_file="output/diff2050_icar_glm.png")
```

```{r diff2050_icar_rf_plot}
far$plot$differences("output/diff2050_icar_rf.tif",
                     borders="data/mada/mada38s.shp",
                     output_file="output/diff2050_icar_rf.png")
```

```{r diff2050_icar_rfxy_plot}
far$plot$differences("output/diff2050_icar_rfxy.tif",
                     borders="data/mada/mada38s.shp",
                     output_file="output/diff2050_icar_rfxy.png")
```


```{r diff2050_rfxy_rf_plot}
far$plot$differences("output/diff2050_rfxy_rf.tif",
                     borders="data/mada/mada38s.shp",
                     output_file="output/diff2050_rfxy_rf.png")
```
